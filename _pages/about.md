...
- <span style="color:#4f6d8c; font-weight:600;">[2025.09‑2026.02]</span> **智屏云播‑商超智能导览屏**: Qt6 embedded device terminal, MQTT sync, incremental resource update, customer verification.
- <span style="color:#4f6d8c; font-weight:600;">[2025.06‑2025.09]</span> **智监易联‑工业分散设备运行状态监测系统**: STM32+ESP8266, sensor data acquisition & MQTT report, workshop site verification.

# <span class="anchor" id="selected-projects"></span>📝 Selected Projects
- <span style="font-size:16px; font-weight:600">智脸枢守‑科研机房人脸识别出入管控 system</span>
  <span style="font-size:14px">**Tan Qingchun**</span>
  <span style="font-size:14px">C++, Qt6, OpenCV, YOLOv8‑face, ONNX Runtime, Dlib, SQLite, MQTT, RK3588, ESP32</span>
  <span style="font-size:14px">Responsible for full Qt device‑side development. Implements face registration, face comparison, EAR blink liveness detection, offline‑local recognition, MQTT heartbeat & end‑cloud data synchronization with id‑based idempotent strategy, ESP32 peripheral interaction. UI‑inference thread decoupling, auto‑degrade when AI model fails. Real‑site deployment & verification in laboratory environment.</span>

- <span style="font-size:16px; font-weight:600">智屏云播‑商超智能导览屏</span>
  <span style="font-size:14px">**Tan Qingchun**</span>
  <span style="font-size:14px">C++, Qt6, CMake, Mongoose, MQTT, SQLite, POSIX message queue, INI config</span>
  <span style="font-size:14px">Responsible for Qt device‑side. Implements embedded HTTP service, device initialization & persistent configuration, MQTT heartbeat & remote upgrade, advertisement resource download with SHA256 integrity check and incremental update. Customer‑side verification and iterative bug‑fix.</span>

- <span style="font-size:16px; font-weight:600">智监易联‑工业分散设备运行状态监测 system</span>
  <span style="font-size:14px">**Tan Qingchun**</span>
  <span style="font-size:14px">STM32L431RC, FreeRTOS, ESP8266, MQTT, multiple sensors</span>
  <span style="font-size:14px">Develop peripheral driver APIs, ESP8266 WiFi & MQTT uplink communication, local audible‑visual alarm, key‑interrupt local configuration. Solved industrial electromagnetic interference problems, verified in workshop site.</span>
